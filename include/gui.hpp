...
class Gui {
    public:
        ...
        void updateBarVal(std::string& bar_name, int new_val, uint new_max_val);
        void updateBarColours(std::string& bar_name, color_t new_fg, color_t new_bg, color_t new_msg_fg);

        void renderChar(uint x, uint y, char chr,color_t fg, color_t bg) const;
        void renderString(uint x, uint y, std::string str, color_t fg, color_t bg) const;
